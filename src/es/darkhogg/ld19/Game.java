...
public final class Game extends Canvas implements Runnable, KeyListener {
	...
	private volatile boolean keyUp, keyDown, keyRight, keyLeft, keyDig, keyChest, keyAttack, keyPause;
	...
	private void update () {
		...
		if ( !gameOver && !titleScreen && !paused ) {
			if ( now - lastMove >= MOVE_TIME ) {
				...
				// Move Up
				if ( keyUp
				  && currentCave.getTerrainAt(
						  (player.posX)/16, (player.posY-MOVE)/16
					 ).isWalkable()
				) {
					player.posY -= MOVE;
				}
				...
				if ( keyLeft
				  && currentCave.getTerrainAt(
						  (player.posX-MOVE)/16, (player.posY)/16
					 ).isWalkable()
				) {
					player.posX -= MOVE;
				}
			...
			if ( now-lastHit > 700000000L ) {
				for ( Enemy enem : enemies ) {
					if ( enem.canHurt()
					  && Math.abs(enem.posX-player.posX) < 16
					  && Math.abs(enem.posY-player.posY) < 16
					) {
						...
						Direction[] dirs = {
							player.facing,
							player.facing.rotateLeft(),
							player.facing.rotateRight(),
							player.facing.invert()
						};
						do {
							player.facing = dirs[ i ];
							i++;
						} while ( !currentCave.getTerrainAt(
								player.posX/16-player.facing.getX(),
								player.posY/16-player.facing.getY()
							 ).isWalkable() && i <= 4
						);
			...
			// Tick spriteentities
			for (
				Iterator<Map.Entry<SpriteEntity,Long>> it = treasureEntities.entrySet().iterator();
				it.hasNext();
			) {
			...
			if ( currentCave.getTerrainAt( player.posX/16, player.posY/16 ) == TerrainType.EXIT ) {
				...
				if ( currentCave.subCaves.get( point ) == null ) {
					currentCave.subCaves.put( point, Cave.generateCave(
						32 + (int)( Math.random()*256 ),
						32 + (int)( Math.random()*256 )
					) );
			...
			// Generate Enemies
			if ( enemies.size() <= Math.sqrt( currentCave.width*currentCave.height )/8
			  && now - lastEnemy > 5000000L
			) {
				lastEnemy = now;
				  ...
				  && difX*difX + difY*difY < 512*512
					...
					if ( currentCave.isDiscovered( tryX/16, tryY/16 ) ) {
						for ( int i = 0; i < 25; i++ ) {
							...
							particles.add(
								new Particle( this, x, y, xs, ys, 0.0, -0.01, spr, life ) );
						}
					}
				}
			}
			...
			for ( Iterator<Enemy> eit = enemies.iterator(); eit.hasNext(); ) {
				...
				for ( Iterator<Arrow> ait = arrows.iterator(); ait.hasNext(); ) {
					...
					if ( enem.canBeHurt()
					  && Math.abs( enem.posX-arrow.posX ) < 16
					  && Math.abs( enem.posY-arrow.posY ) < 16
					) {
						enem.life--;
						ait.remove();
						Sound.HURT.play();
			...
			for ( Iterator<Heart> it = hearts.iterator(); it.hasNext(); ) {
				Heart heart = it.next();
				if ( Math.abs( heart.posX-player.posX ) < 16
				  && Math.abs( heart.posY-player.posY ) < 16
				) {
			...
			for ( Iterator<Ball> it = balls.iterator(); it.hasNext(); ) {
				Ball ball = it.next();
				if ( Math.abs( ball.posX-player.posX ) < 8
				  && Math.abs( ball.posY-player.posY ) < 8
				) {
					player.life -= 1;
					it.remove();
					Sound.HURT.play();
				...
				for ( int j = minY/16-1; j <= maxY/16; j++ ) {
					if ( currentCave.isDiscovered( i, j ) ) {
						g.setColor( new Color( 0, 0, 0,
							(int)( (0.8-0.8*currentCave.getLightAt( i, j ))*255 ) ) );
						int x = i*16 - camX + WIDTH/2 - 8;
						int y = j*16 - camY + HEIGHT/2 - 8;
						g.fillRect( Math.max(x,0), Math.max(y,0),
							x+16-Math.max(x,0), y+16-Math.max(y,0) );
					}
				}
			}
